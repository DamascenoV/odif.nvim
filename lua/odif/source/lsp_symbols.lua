...
return {
  ...
  items = function(set)
    ...
    if #clients == 0 then
      vim.notify('[odif] no LSP client supports documentSymbol',
        vim.log.levels.WARN)
      set({})
      return
    end
    ...
    vim.schedule(function()
      vim.lsp.buf_request_all(bufnr, 'textDocument/documentSymbol', params,
        function(results)
          local items = {}
  ...
  format_item = function(it)
    return ('[%s] %s'):format(it.kind, it.name)
  end,
  choose = function(it)
    ...
    if it.uri then
      vim.cmd.edit(vim.uri_to_fname(it.uri))
    end
