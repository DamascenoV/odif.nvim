...
local got
match.run_async(items, 'fb', true, { chunk_size = 4000 }, function(matches) got = matches end)
...
local t = match.run_async(items, 'fb', true, { chunk_size = 1000 }, function() cancelled_called = true end)
t.cancel()
...
assert(ident and #ident == 3 and ident[1] == 1 and ident[3] == 3, 'empty query path broken')
